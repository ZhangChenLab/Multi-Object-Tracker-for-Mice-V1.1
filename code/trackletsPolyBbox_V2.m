...
%   tracks       追踪轨迹记录
% % -----------------------------
%   last version modify by feng su 
%   20190514 | generate low-level tracklets
% % -----------------------------
...
assignments_T=[];
ThreIoU=[0.8 0.85];        % 越小越难assign
ThreIoUDiff=[0.15 0.1];  % 越大越难assign
centroids=squeeze(mean(bboxes_ForAssign(:,[1 3],:),2));  % 检测bbox的中心
...
for assign_i=1 % :2    % 2轮assign，是否必要？
    ...
    for detec_i=1:nDetections
        ...
        % 如果一个detection bbox对应多个track,重新校正cost
        TRCost=cost(:,detec_i);
...
%%  % createNewTracks
if(~isempty(unassignedDetections))  % 未分配的检测和丢失的ID都非空
    centroids_1 = centroids(unassignedDetections, :); % 未分配的检测 | centroid
    polyareas_1 = polyareas(unassignedDetections); % 未分配的检测 | area
    polybboxes_1 = PolyBboxes(unassignedDetections,:, :); % 未分配的检测 | bbox
    unassignedDetectionsL=length(unassignedDetections); % 未分配的检测个数
